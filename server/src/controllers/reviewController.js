...
const updateReviewSchema = reviewSchema.fork(['courseCode', 'rating'], (field) => field.optional());

function normalizeReviewPayload(payload) {
  const clean = { ...payload };
  if (clean.comment === undefined || clean.comment === null) clean.comment = '';
  if (clean.reviewedBy === undefined || clean.reviewedBy === null || clean.reviewedBy === '') {
    clean.reviewedBy = null;
  }
  return clean;
}
...
export async function getCourseSummary(req, res, next) {
  try {
    ...
    const target = String(courseCode).trim().toUpperCase();
    const summary = await Review.aggregate([
      { $match: { courseCode: target } },
      {
        $group: {
          _id: '$courseCode',
    ...
    if (!summary.length) {
      return res.status(404).json({ message: `No reviews found for course ${target}` });
    }
